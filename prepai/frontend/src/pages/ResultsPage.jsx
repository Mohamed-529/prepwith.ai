    ...
    <div className="flex flex-col items-center gap-3">
      <div className="relative w-32 h-32">
        <svg className="w-full h-full -rotate-90" viewBox="0 0 120 120">
          <circle cx="60" cy="60" r="50" fill="none" stroke="#e8efe9" strokeWidth="10" />
          <circle
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-4">
        <div className="card text-center max-w-sm">
          <p className="text-slate-600 mb-4">No results found. Please complete an interview first.</p>
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="text-center mb-10 animate-slide-up">
        <h1 className="text-3xl md:text-4xl font-bold text-slate-800 mb-2">Interview Complete!</h1>
        <p className="text-slate-500">Here's how you performed across all questions.</p>
      </div>

      {/* Score card */}
      <div className="card mb-8 text-center bg-gradient-to-br from-white via-brand-50 to-peach-50 shadow-lg shadow-mist-400/20 animate-fade-in">
        <ScoreGauge percentage={percentage} />
        <div className="mt-6 grid grid-cols-3 gap-4">
          <div className="bg-emerald-50/80 rounded-xl p-4 border border-emerald-100">
            <p className="text-2xl font-bold text-slate-800 font-display">{score}</p>
            <p className="text-xs text-slate-500 mt-1">Correct</p>
          </div>
          <div className="bg-rose-50/80 rounded-xl p-4 border border-rose-100">
            <p className="text-2xl font-bold text-slate-800 font-display">{totalQuestions - score}</p>
            <p className="text-xs text-slate-500 mt-1">Incorrect</p>
          </div>
          <div className="bg-mist-100 rounded-xl p-4 border border-mist-200">
            <p className="text-2xl font-bold text-slate-800 font-display">{totalQuestions}</p>
        ...
        {(answers || []).map((ans, idx) => (
          <div
            ...
            className={`card border transition-colors cursor-pointer ${
              ans.isCorrect ? "border-emerald-200 hover:border-emerald-300" : "border-rose-200 hover:border-rose-300"
            }`}
            onClick={() => setExpandedIdx(expandedIdx === idx ? null : idx)}
          >
            <div className="flex items-start justify-between gap-4">
              <div className="flex items-start gap-3 flex-1 min-w-0">
                <div className={`w-7 h-7 rounded-full shrink-0 flex items-center justify-center mt-0.5 ${ans.isCorrect ? "bg-emerald-50 text-emerald-600" : "bg-rose-50 text-rose-600"}`}>
                  {ans.isCorrect
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-slate-800 leading-snug">{ans.questionText}</p>
                  <p className="text-xs text-slate-400 mt-1">Q{idx + 1} · Click to {expandedIdx === idx ? "collapse" : "expand"}</p>
                </div>
              </div>
            ...
            {expandedIdx === idx && (
              <div className="mt-4 pt-4 border-t border-mist-200 space-y-4 animate-fade-in">
                <div>
                  <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2">Your Answer</p>
                  <p className="text-sm text-slate-600 leading-relaxed bg-mist-50 rounded-xl px-4 py-3 border border-mist-200">
                    {ans.userAnswer?.trim() || <span className="text-slate-400 italic">No answer provided</span>}
                  </p>
                </div>
                <div>
                  ...
                  <p className="text-sm text-slate-600 leading-relaxed bg-brand-50 rounded-xl px-4 py-3 border border-brand-100">
                    {ans.feedback}
                  </p>
                </div>
              </div>
