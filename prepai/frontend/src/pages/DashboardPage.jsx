  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="mb-10 animate-slide-up">
        ...
        <p className="text-slate-500">Track your progress and keep practicing to land your dream job.</p>
      </div>

      {/* Stats */}
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-10">
          {[1,2,3].map(i => <div key={i} className="card h-24 animate-pulse bg-mist-100" />)}
        </div>
      ) : (
      ...
      {/* CTA */}
      <div className="card mb-10 bg-gradient-to-br from-brand-50 via-white to-peach-50 border-brand-100">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div>
            <h2 className="text-xl font-bold text-slate-800 mb-2">Ready for your next mock interview?</h2>
      ...
      <div>
        ...
        {loading ? (
          <div className="space-y-3">
            {[1,2,3].map(i => <div key={i} className="card h-16 animate-pulse bg-mist-100" />)}
          </div>
        ) : history.length === 0 ? (
          <div className="card text-center py-12">
            <div className="w-14 h-14 rounded-2xl bg-mist-100 flex items-center justify-center mx-auto mb-4">
              <svg className="w-7 h-7 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /></svg>
            </div>
            <p className="text-slate-700 font-medium mb-1">No sessions yet</p>
            <p className="text-slate-400 text-sm">Complete your first mock interview to see results here.</p>
          </div>
        ) : (
          <div className="space-y-3">
              ...
              return (
                <div key={session._id || idx} className="card flex items-center justify-between gap-4 py-4">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-xl bg-mist-100 flex items-center justify-center text-sm font-bold text-slate-600 font-mono">
                      #{history.length - idx}
                    </div>
                    <div>
