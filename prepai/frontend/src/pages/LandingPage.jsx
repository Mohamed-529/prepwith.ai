...
const Feature = ({ icon, title, desc, tone }) => (
  <div className={`card hover:border-brand-200 transition-colors group ${tone.card}`}>
    <div className={`w-10 h-10 rounded-xl border flex items-center justify-center mb-4 transition-colors ${tone.icon}`}>
      {icon}
    </div>
...
const tones = {
  mint: { card: "bg-gradient-to-br from-white to-brand-50/70", icon: "bg-brand-50 border-brand-100 text-brand-600 group-hover:bg-brand-100" },
  peach: { card: "bg-gradient-to-br from-white to-peach-50/80", icon: "bg-peach-50 border-peach-100 text-peach-500 group-hover:bg-peach-100" },
  sand: { card: "bg-gradient-to-br from-white to-mist-100/80", icon: "bg-mist-100 border-mist-200 text-brand-700 group-hover:bg-mist-200" },
};

const LandingPage = () => (
  <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
    <div className="py-24 text-center animate-slide-up">
      <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/80 border border-brand-100 text-brand-700 text-xs font-medium mb-8 shadow-sm">
      ...
      <h1 className="text-5xl md:text-6xl lg:text-7xl font-bold text-slate-800 mb-6 leading-tight">
        Ace your next<br />
        <span className="text-transparent bg-clip-text bg-gradient-to-r from-teal-700 via-cyan-600 to-orange-500">
          tech interview
        </span>
      </h1>
